...
PcapWriter_init(PcapWriter *self, PyObject *args, PyObject *kwds)
{
    ...
    // get filename from passed python object
    if(!PyUnicode_Check(filename)){
        PyErr_SetString(PyExc_ValueError, "Argument 'filename' must be a string object");
        return -1;
    }
    char *_filename = PyUnicode_ToString(filename);
    if(_filename == NULL){
        PyErr_SetString(PyExc_ValueError, "Could not parse filename into C string");
        return -1;
    }
...
static int
PcapWriter_set_filename(PcapWriter *self, PyObject *value, void *closure){
